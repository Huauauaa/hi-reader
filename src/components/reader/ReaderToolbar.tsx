...
function activeCls(on: boolean): string {
  return on
    ? 'bg-[color-mix(in_srgb,var(--page-ink)_18%,transparent)]'
    : 'bg-[color-mix(in_srgb,var(--page-ink)_8%,transparent)]'
}
  ...
  return (
    <div className="absolute right-3 top-1/2 z-30 flex -translate-y-1/2 flex-col gap-2.5 md:right-5">
      <button
        ...
        className={`${btn} ${activeCls(active === 'toc')}`}
        onClick={() => onToggle('toc')}
      >
        <List size={20} />
      </button>
      <button
        type="button"
        aria-label="字号"
        aria-pressed={active === 'font'}
        className={`${btn} ${activeCls(active === 'font')}`}
      ...
      <button
        ...
        className={`${btn} ${activeCls(active === 'notes')}`}
      ...
      <button
        ...
        className={`${btn} ${activeCls(active === 'theme')}`}
