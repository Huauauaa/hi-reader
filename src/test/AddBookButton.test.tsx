...
describe('AddBookButton', () => {
  it('toasts on unsupported format', () => {
    const onAdded = vi.fn()
    const onToast = vi.fn()
    const { container } = render(<AddBookButton onAdded={onAdded} onToast={onToast} />)
  ...
  it('toasts when addLocal fails', async () => {
    vi.spyOn(booksStore, 'addLocal').mockRejectedValueOnce(new Error('IDB error'))
    const onAdded = vi.fn()
    const onToast = vi.fn()
    const { container } = render(<AddBookButton onAdded={onAdded} onToast={onToast} />)
  ...
  it('opens file picker on button click', () => {
    const { container } = render(<AddBookButton onAdded={() => {}} onToast={() => {}} />)
    const input = container.querySelector('input[type="file"]') as HTMLInputElement
